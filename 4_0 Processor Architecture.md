...
* Pipeline processor:
	* This processor breaks the execution of each instruction into 5 steps, each of which is handled by a separate section or stage of the hardware.
	* Instructions progress through the stages of the pipeline, with one instruction entering the pipeline on each clock cycle.
	* The processor can be executing the different steps of up to 5 instructions simultaneously.
